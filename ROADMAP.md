...
- [x] **Phase B — Gmail read + triage** — `src/lib/gmail.server.ts` gains read (`messages.list` + metadata), label (`labels` + `messages.modify`) and `drafts.create` on the existing OAuth credential, plus `src/lib/triage.server.ts`: recent unread mail is classified (category, priority, needs-reply, reason), labelled `AiXin/<priority>`, and a reply is **staged as a Gmail draft**. Nothing is ever sent — `sent: 0` is recorded in the artifact and the outcome says so explicitly. A token without `gmail.modify` yields a verbatim Google 403 and a `GUARD` event instead of a fake triage; no live Gmail connection blocks the run rather than simulating it. New SIP action `email_triage` (low risk — staging a draft is not an external effect).
- [x] **Phase C — Solo-business money jobs (invoice & payment chase)** — new `invoices` table (user-scoped RLS + grants, org guard, due-date index) and `src/lib/money.server.ts`. The chase finds every unpaid invoice past its due date and builds the reminder **deterministically from stored invoice facts** (number, amount, currency, due date, days overdue) — the model never writes the money message — then stages it as an email draft and increments `chase_count`. New SIP action `invoice_chase` is **high risk**, so it always produces a Decision Card and only runs after a human signature; `lead_followup` is registered as medium risk for the next job. UI at `/dashboard/money`: invoice list with overdue badges, add/edit, mark paid, and "Run now" for both the chase and inbox triage. Fully EN/ZH.
- [x] **Phase C (remaining) — lead follow-up + overnight business watch** — new `leads` table (owner/org RLS, grants, org guard) and `src/lib/leads.server.ts`: the follow-up builds each reply **deterministically from stored lead facts only** (name, source, enquiry, owner booking URL) — no price, date or availability is ever invented — stages it as an email draft, and only then increments `followup_count`/status. `lead_followup` stays **medium risk**, so it waits for a signature. `src/lib/watch.server.ts` adds a **read-only** overnight watch that counts failed deliveries, blocked/awaiting tasks, unsigned Decision Cards, failed anchors, overdue invoices and unanswered enquiries, returns severity-sorted alerts, and records `changed_anything: false`; registered as low-risk `business_watch` so it is safe to schedule unattended. UI: enquiry list, both job cards and an instant "Check now" watch preview on `/dashboard/money`. Order/refund exception escalation remains open.
- [x] **Phase D — Cited customer/community Q&A** — new `qa_sources` / `qa_answers` tables and `src/lib/qa.server.ts`: answers are drawn **only** from the owner's enabled sources (max 12, 4 000 chars each), every claim must cite a source ID, cited IDs are validated against the loaded sources, and an uncovered question is **refused in code** rather than improvised. Citation counts per source are tracked, and every question — answered or refused — is recorded. UI at `/dashboard/qa`, fully EN/ZH. Referral-conversion tracking remains open.
- [x] **Phase E — Governed memory** — new `memory_blocks` table and `src/lib/memory.server.ts`: every block carries a **purpose**, an owner-written label, optional retention converted to an explicit `expires_at`, an enable switch, and use counters. Retrieval returns only enabled, unexpired blocks matching the requested purpose, and each read is counted and stamped. Inbox triage now reads only `email_triage` + `tone_of_voice` blocks and logs a `MEMORY` trace event naming the blocks used. Memory is rendered to the model as labelled facts, never as instructions. Inspect, pause, edit, delete or forget-a-whole-purpose at `/dashboard/memory`, fully EN/ZH.
- [x] **Phase F — Outcome catalogue (time-to-first-real-outcome)** — user feedback was that authoring a Skill is a five-step exercise that often ends in a generic report, so the primary entry point is now **what I can do for you** at `/dashboard/jobs`, driven by `src/lib/outcomes.ts` (six outcomes, each with a real executor branch, risk tier, approval rule and the requirements it needs). `src/lib/outcomes.functions.ts` does one-tap setup: reuse-or-create the Specialist (with ERC-8004 identity), create-or-reuse the skill by stable slug (`outcome-<id>`, snapshotted in `skill_versions`), install and assign — all idempotent — then `runOutcome` delegates through `delegateThroughSip`, so a one-tap job is governed exactly like a chat delegation. The board reports the one thing still missing (unpaid invoice, live Gmail, linked Telegram, approved source) instead of failing silently. SkillCraft becomes the **Advanced** door and now refuses honestly at creation time: if the derived SIP action has no executor (`hasExecutor` in `skill-manifest.ts`), the publish step warns that nothing behind AiXin can carry the job out yet and that a run will stop and say so rather than invent a result. Fully EN/ZH.
- [x] **Phase G — Public proof page** — fundraising needs traction anyone can quote, so `/proof` is a public, bilingual **live proof** page: real-time counters of governed actions, Ed25519-signed receipts, BSC Testnet anchors, receipts carrying ERC-8004 agent identity, human decisions honoured (approved / rejected / pending) and draft-only email replies staged (emails sent by AiXin: 0, by design). Served by `GET /api/public/proof`, which aggregates existing `receipts` and `decision_cards` tables with the service role and returns counts only — no user ids, payloads or PII (60s cache, CORS open). The page deep-links to verify the latest receipt and to the validator public key, and shows honest zeros rather than invented numbers. Linked from the landing footer.
- [x] **Phase G.1 — Pilot case-study generator** — every pilot can now quote their own traction: `/dashboard/jobs` ends with a bilingual **Your results so far** card (`getPilotCaseStudy`) that aggregates the owner's real `tasks`, `receipts`, `decision_cards`, `invoices` and `leads` into governed runs completed, signed/anchored receipts, approvals honoured, invoice reminders and lead follow-ups drafted, and emails sent (always 0, by design). A one-tap **Copy my case study** button produces the paragraph for pilot reports and investor updates. Time saved is shown only as a clearly-labelled estimate with its assumption (15–30 min per completed job) stated next to it — no invented numbers.
- [x] **Phase G.2 — Developer surface (Astara Twin path), honestly scoped** — the headless API for builders is partially live today: `POST /api/public/v1/generate` and `POST /api/public/v1/safety-check` are public, API-key-authenticated endpoints over the same SIP validation the app uses, and `/api/public/verify/:sipId` plus `/api/public/proof` are open reads. Still planned: per-developer keys with scopes/rate limits, run submission with Decision Card webhooks, and SDK packaging — tracked under Phase 4 W5. Never describe the developer API as fully shipped until those land.
- [x] **Phase G.3 — Two-minute first outcome (onboarding)** — hatching the Master Twin no longer drops a new owner on an empty dashboard. Onboarding gains a fourth step, **your first result**, listing only the outcomes whose `requires` list is empty (overnight business watch, price outlook) — the two jobs that need nothing else connected. One tap runs the same governed path as everywhere else (`setupOutcome` -> `runOutcome` -> `delegateThroughSip`): the Specialist is created, the skill installed and assigned, the run executed, and the owner is taken straight to the task with its signed receipt. Each card still states its honest limit before it is pressed, and the step can be skipped straight to `/dashboard/jobs`. Fully EN/ZH.
- [x] **Phase G.4 — Plain-language rules (skill guardrails without syntax)** — the Rules step of SkillCraft asked beginners to hand-write predicates like `requires_approval_if amount > 1000`, which is the single biggest authoring drop-off. It is now toggles and numbers: always ask me first, never send (leave a draft), office hours only, answer only from approved material, known contacts only, ask me before spending more than N, most items per run N. `src/lib/plain-rules.ts` renders the choices into the same deterministic predicate text SIP validates (`renderPlainRules`), reads existing text back into the toggles (`parsePlainRules`) so AI drafts, blueprints and edits stay in sync, and an **Advanced: edit the rule text myself** switch keeps hand-authoring for builders. Unit-tested (`plain-rules.test.ts`), fully EN/ZH. `AIXIN_USER_GUIDE.md` gained a bilingual **what's new and improved** changelog table covering this and everything shipped in the last three weeks.
- [x] **Phase G.5 — Telegram as a full front door (ask, run and approve by message)** — the fastest surface a solo owner already lives in is chat, so the linked Telegram chat is now a first-class control room, not just a notification channel. `src/lib/telegram-front.server.ts` handles `/help`, `/jobs` (numbered outcomes with their per-owner missing requirements), `/run <n> [note]`, `/pending`, `/approve <code> [reason]` and `/reject <code> <reason>`; anything else still falls through to the task thread. Crucially there is **no cheaper path**: the approve/reject flow was extracted verbatim into `src/lib/decide.server.ts` (`decideOnCardCore` — Ed25519 signing, BSC anchoring, ERC-8004 identity/reputation/validation, receipt insert, ledger and reputation writes) and one-tap setup/run into `src/lib/outcomes.server.ts` (`setupOutcomeCore`/`runOutcomeCore`), with the authenticated server functions now thin wrappers over the same cores. A rejection without a reason is refused in chat. Outcome skill slugs are now owner-suffixed (`outcome-<id>-<uid8>`, legacy unsuffixed slugs still read) because `skills.slug` is globally unique and the first owner of a job was blocking everyone else's one-tap setup. Verified end to end against the live webhook and database: `/jobs`, `/run 3`, `/approve <code>` produced a governed run with a signed, anchored receipt. Fully EN/ZH (locale detected from the message).
- [x] **Phase G.6 — Developer surface, exception escalation and referral tracking** — the headless API is now governed, not just authenticated. `org_api_keys` carries `scopes` (`generate` / `safety_check` / `runs`), `rate_limit_per_min` (default 60, enforced from a rolling 60-second count in the new `api_requests` table, `429` beyond it), `webhook_url` / `webhook_secret`, and `owner_user_id` — the accountable human an API run is signed for; a key without an owner cannot submit runs (`409`), because AiXin will not take an action it cannot attribute to a person. `POST /api/public/v1/runs` accepts a catalogue job or free-text intent and `GET /api/public/v1/runs/{id}` returns the run, its Decision Card and its receipt with the public verify link; both go through the same `setupOutcomeCore` / `runOutcomeCore` / `delegateThroughSip` cores, so there is no cheaper path than the dashboard — verified end to end (a low-risk watch completed; a refund intent with an amount returned `awaiting_approval` with a Decision Card). `src/lib/developer-webhooks.server.ts` delivers `run.created`, `decision_card.created`, `decision_card.approved` and `decision_card.rejected` signed with the existing `X-AiXin-Signature: t=…,v1=…` HMAC scheme, wired into `decideOnCardCore` so no decision path skips notification; delivery is best-effort and never blocks a governed action. `sdk/aixin.ts` (zero-dependency TypeScript client with `verifyWebhook`) and bilingual `DEVELOPER_API.md` ship with it; npm packaging, a Python client and per-key usage dashboards stay 🔵 planned. Two older open items also closed: a refused refund now calls `raiseEscalation()` (new `escalations` table, owner-only RLS), adds an `ESCALATED` trace event and appears as **Needs a human** on the Business jobs page with acknowledge / mark-handled / reopen — nothing paid out, nothing improvised; and a staged lead follow-up records a referral (new `referral_conversions` table, owner-only RLS, one row per lead per source) shown on the same page with counts and only the value the owner enters themselves. Collateral: `AIXIN_FUNDING_DECK.md` — a 10-slide bilingual deck carrying the live proof counters, the priced-problem evidence with sources, the three-surface story (consumers, businesses, developers), the honest seed-scale ask and the remaining 🔴 traction gaps. Fully EN/ZH.
- [ ] **Phase C/D (remaining) — order/refund exception escalation and referral-conversion tracking.**
- [ ] **Real inbound phone calls** — requires a telephony provider (Twilio or a domestic Chinese carrier) plus number filing; Telegram voice notes ship first and stay the default demo. Deferred by owner on cost grounds (2026-09-18); voice coverage today = `/voice` in Telegram plus the scheduled voice-note briefing.
- [x] **Phase G.7 — Tier 2 adapter batch (shipped 2026-09-19)** — Gmail was the only email source; every provider below now has a curated adapter card, a **Test connection** button that performs a real provider round-trip, and typed failure codes (`missing_config`, `bad_credentials`, `auth_error`, `not_found`, `http_error`, `network_error`, `unsupported_runtime`) so a misconfigured connection can never present as "connected". Probes live in `src/lib/tier2.server.ts` and are read-only (the one exception is DingTalk, which has no read endpoint — its probe posts a single fixed, labelled test line and never owner data).
  - [x] **Microsoft 365 / Outlook** — Graph app-only (`src/lib/outlook.server.ts`): reads unread inbox mail, marks priority as an Outlook category, creates reply drafts. **No send function exists in the module.** Inbox triage now runs on Outlook exactly as on Gmail via the shared `finishTriage` half of `src/lib/triage.server.ts`, so the draft-only guarantee cannot drift between providers; `execution.server.ts` prefers Gmail, falls back to Outlook, and otherwise refuses with "No live Gmail or Microsoft 365 mailbox connection — the inbox was never read."
  - [x] **IMAP generic mail** — adapter card + real TLS `LOGIN` + `STATUS INBOX` probe. Honest limit: the Worker runtime has no raw sockets, so the probe returns `unsupported_runtime` there rather than pretending to connect; it verifies on a self-hosted/Node deployment.
  - [x] **WeChat Work (企业微信)** — `gettoken` + department read probe against `qyapi.weixin.qq.com`.
  - [x] **Feishu / DingTalk** — Feishu `tenant_access_token` probe; DingTalk HMAC-SHA-256 signed webhook probe.
  - [x] **Google Sheets** — API-key probe against the named spreadsheet + range; a `403` is reported as "sheet not shared", not as a generic failure.
  - [x] **Outbound webhooks v2** — managed retry queue (`webhook_retries` table + `src/lib/webhook-queue.server.ts`): permanent failures (no URL, insecure URL, 4xx other than 429) are never queued; transient failures park and are re-attempted on the existing 15-minute assistant tick with 1/5/15/60/180-minute backoff, re-reading the owner's signing secret each attempt so a rotated secret applies immediately. Task outcome deliveries route through it.
  - [x] **Stripe / invoicing reads (shipped 2026-09-18)** — `src/lib/stripe.server.ts` is read-only by construction (no charge, refund, void or write call exists in it). `testStripe` performs a real round-trip against `/v1/balance` + `/v1/account` and returns typed failure codes (`no_key`, `bad_key`, `auth_error`, `http_error`, `network_error`) so a misconfigured key never presents as "connected". `listStripeOpenInvoices` fetches `status=open` only, converts minor units to major with a zero-decimal-currency exemption list, and dedupes by invoice number. `importStripeInvoices` refreshes amount / due date / status on invoices already present **without resetting their chase counters** and inserts the rest, so the invoice-chase job runs off real Stripe records instead of hand-typed rows. Surfaced as a curated adapter card (secret key + account label) with a **Test connection** button that reports the account label and the number of unpaid invoices found, plus an **Import from Stripe** button on the Business jobs page. Fully EN/ZH.
- [x] **Phase G.9 — Shopify adapter: order watch + approval-gated refunds (shipped 2026-09-19)** — the answer to "which meaningful platform permits automation": Shopify's Admin API explicitly invites it, so no credential storage, no browser impersonation, no terms breach. `src/lib/shopify.server.ts` normalises the store domain (`*.myshopify.com` only), calls the Admin API with `X-Shopify-Access-Token`, and returns typed failure codes (`missing_config`, `bad_domain`, `auth_error`, `not_found`, `http_error`, `network_error`). Two capabilities, deliberately separated:
  - [x] **Order watch (read-only, low risk)** — `src/lib/shopify-watch.server.ts`: deterministic alerts counted from the store itself — paid-but-unshipped for ≥48h, awaiting fulfilment inside the normal window, payments that never arrived, part-refunded orders (likely disputes), fully refunded orders, plus the total value still waiting to ship. Artifact carries `changed_anything: false`. Surfaced as the **Watch my shop orders** outcome (`shopify_order_watch`, requirement `shopify`) with one-tap setup, specialist "Shopfloor / 店务", and a runnable executor branch in `execution.server.ts`; refuses with "No live Shopify connection — the store was never read." rather than inventing a report.
  - [x] **Refunds (high risk, always approval-gated)** — `src/lib/shopify-refund.server.ts`: before approval it gathers real evidence from the store (order total, already-refunded amount, still-refundable balance, customer, flags) so the Decision Card shows facts, not paraphrase; `startIntent` forces risk `high` for any live-store refund. After approval it **re-reads the order and re-caps the amount against what is still refundable** — an over-approval is refused, nothing is paid, and the refusal is escalated to the Business jobs page. Refund is created through Shopify's own `refunds` endpoint (with `refunds/calculate` first, so the refundable transaction is never guessed); the module cannot cancel, edit or re-fulfil an order.
  - [x] Curated adapter card (store domain + Admin API token + optional API version) with a real **Test connection** probe registered in `TIER2_PROBES`; removed from the "coming soon" marketplace list. Fully EN/ZH. Demo script gained **Act 2c** and the user guide a full worked shop example (**Step 5c / 第 5c 步**) with the exact Shopify scopes to tick.
- [x] **Phase G.10 — The store team: cross-adapter outcomes + the beginner walkthrough (shipped 2026-09-20)** — the answer to "I can see the features, but what would my twin actually *do* for me?". Deliberately **not** a "profitable store" demo: AiXin does not pick products, price them, buy ads, post marketing or promise profit, and the page says so in both languages. Instead the Master Twin coordinates four Specialists — Shopfloor (Shopify), Money (Stripe + invoice records), Inbox (Gmail/Outlook), Chief of Staff (Telegram) — over the work that actually decides profitability.
  - [x] **My store morning round (`store_morning_round`, low risk, no approval)** — `src/lib/store-round.server.ts`: one read-only artifact aggregating four sources by reusing existing readers (`shopify-watch.server.ts`, `stripe.server.ts` open invoices + the `invoices` table, the unread-read half of the mail readers — no label, no draft — plus pending Decision Cards, running tasks, delivery failures and open escalations). Per-source degradation is reported verbatim ("not connected" / the provider's own error), never dropped and never invented; the artifact carries `changed_anything: false` and a `needs_you` count. Delivered through the existing 07:30 assistant schedule as Telegram text + voice note.
  - [x] **Bring my past customers back (`winback_customers`, high risk, always approval-gated)** — `src/lib/winback.server.ts`: deterministic `selectWinbackCandidates` (exactly one paid, non-cancelled, non-refunded order per email; ≥60 days quiet; newest-lapsed first; per-run cap 10) and `winbackMessage` built from order facts only — no discount, price or date is ever invented. Drafts are created in Gmail only (no send path imported), deduped by the `winback_drafts` table (`UNIQUE (user_id, customer_email)`, owner-scoped RLS) so a customer can never be written to twice, even across daily runs.
  - [x] **The beginner walkthrough** — `src/lib/recipe.ts` + `src/routes/dashboard.recipe.tsx`: the ten-step build as an in-app checklist with **live** status per step read from the existing readiness checks (connection linked / job set up), so it cannot claim a step is done when it is not; each step names the screen, the button, what success looks like and what failure looks like. Includes the "Make it yours" adapt table (clinic, studio/agency, wholesaler, anything else) and the judging rule: no connection that can read the facts + no way to check the result afterwards = not a job for a twin yet. Sidebar entry **Build the store team / 搭建店铺团队** and a link from the Jobs page.
  - [x] Wired through the same rails as every other outcome: `sip.server.ts` `KNOWN_ACTIONS` (+ `HIGH_RISK_ACTIONS` for win-back), `EXECUTOR_BACKED_ACTIONS`, `deriveSipAction`, `execution-capability.ts`, `execution.server.ts` executor branches, `outcomes.ts` one-tap setup. Fully EN/ZH. Tests cover partial-connection reporting, the 60-day filter, the per-run cap and the dedupe (116 library tests passing). Demo script gained **Act 2d / 第二幕 d** and the user guide **Step 5d / 第 5d 步**.
- [x] **Phase G.11 — Primitive capability layer (shipped 2026-09-19; AIXIN_VS_PAA.md Priority 1)** — breaks the capability = skill = executor fusion. Three layers now exist explicitly: jobs (unbounded intent), skills (open, user-authorable, no authority), primitives (~16 gated capabilities that carry all authority).
  - [x] **The catalog** — `src/lib/primitives.ts`: 16 primitives (`read_invoices`, `read_stripe_invoices`, `read_leads`, `read_shopify_orders`, `read_inbox_unread`, `read_market_data`, `read_business_signals`, `compute_forecast`, `compose_summary`, `send_telegram_message`, `create_email_draft`, `label_emails`, `send_email`, `issue_refund`, `set_schedule`, `ask_human`), each with kind, risk class, required provider, and a `runnable` flag, bilingual labels and descriptions. Write primitives that move money or send mail (`send_email`, `issue_refund`, `create_email_draft`, `label_emails`) are catalogued but NOT generic-runnable — they stay behind their dedicated governed executors.
  - [x] **Compositions + the runnable guard** — `src/lib/skill-compositions.ts`: the eleven executor-backed SIP actions are expressed as reference compositions (zero user-visible change); `assessComposition` statically validates any declared composition (unknown ids, explicit misses, required providers, max risk); `skillRunnableVerdict` answers "can this skill do real work today?" via executor OR composition; a partially runnable composition names its misses instead of faking them.
  - [x] **The generic composition runner** — `src/lib/primitives.server.ts`: executes a user-authored skill's composition end to end with no new code per skill — reads pull real data (invoice ledger, leads, business signals, CoinGecko, deterministic forecast, Shopify, Stripe, Gmail unread), `compose_summary` writes the outcome from that data only (LLM metered via `run_usage`; deterministic fallback when no model is configured), `send_telegram_message` delivers it. Unrunnable steps are recorded as explicit ⚠ misses in the outcome and task events.
  - [x] **Wired through the same rails** — new fallback branch 1l in `execution.server.ts` (task's skill → latest `skill_versions.primitives` → runner; outcome, content-safety screen, delivery, receipt and Telegram notify unchanged); `skill_composition` added to `ToolRun`; migration `0007_add_skill_version_primitives` (`skill_versions.primitives jsonb`, nullable); `createSkill`/`updateSkill` persist compositions (an edit that doesn't touch the picker keeps the existing composition); SkillCraft's AI drafter proposes compositions from the catalog.
  - [x] **SkillCraft UI** — the intent step gained the primitive picker (bilingual labels, risk badges, "governed" tags, live run-order preview); the publish step's guard is composition-aware: runs-end-to-end note / partial-with-named-misses note / the original "nothing can carry this out" warning. Fully EN/ZH. 129 library tests passing (13 new), typecheck clean.
- [x] **Phase G.12 — Layman-first simplification: "hide the vocabulary, keep the moments" (shipped 2026-09-19)** — UI and copy reorganisation only; no execution, SIP, risk-tier or executor changes.
  - [x] **Landing + navigation** — sign-in and onboarding now land on Jobs ("What I want done" / 我要办的事) instead of Ask AiXin. Sidebar regrouped into Daily (Jobs, Activity, Money, Scheduled runs, Q&A, Memory), Trust (Governance, Evidence ledger, Connections) and a new always-last **Advanced** group (Command Center, My team, Skills, Capabilities, Recipe walk-through, Organisation, Committer, Reputation) collapsed by default on desktop and mobile.
  - [x] **Vocabulary pass (EN + ZH)** — Specialists → My team (我的团队), Tasks → Activity (动态), Adapters → Connections (连接), Shared Ledger → Evidence ledger (证据账本), SKILLS DEPLOYED → JOBS RUNNING (运行中的工作); Jobs-page copy no longer uses the words "skill"/"builder" in the primary path. No URL changes; every existing route still resolves.
  - [x] **First-run hint** — brand-new workspaces see a plain-language pointer on Jobs ("ask in your own words, or pick a job — AiXin sets everything up") instead of an empty board.
  - [x] **Moments copy** — approval-card buttons now read "Yes, do it / No, stop it" (同意，执行 / 不，先停下); refusal, receipt and queued wording rechecked plain. Skills-guard text cross-references updated to the new Jobs labels.
  - [x] **Tutorial** — AIXIN_SKILL_TUTORIAL.md Fundamentals (EN/ZH) now explains the Advanced door: the vocabulary is optional, the moments are not.
  - [ ] Remaining from AIXIN_VS_PAA.md: Priority 3 known-targets allowlist; Priority 4 escalation-pair metrics; then standing grants, trust labels, miss log, three-valued outcomes.
- [x] **Phase G.13 — Compensation registry + undo (shipped 2026-09-20; AIXIN_VS_PAA.md Priority 2)** — the reverse of an action is decided and written down *before* the action runs; nothing here simulates an undo.
  - [x] **The registry** — `src/lib/compensation.ts`: every SIP action classified `not_needed` / `reversible` / `irreversible`, with a named reverse action (`delete_created_drafts`, `disable_schedule`, `send_correction_message`), an undo window, and bilingual plain-language plans. Unknown actions fall back to irreversible (fail-safe). `mayRunUnattended` is false for anything irreversible.
  - [x] **Recorded before execution** — `recordCompensation` writes the plan into the new `compensations` table on both paths (auto-executed routine runs in `sip.functions.ts`, approved Decision Cards in `decide.server.ts`); `finalizeCompensation` then attaches the real reverse handles (Gmail draft ids, schedule id) extracted from the run artifact.
  - [x] **Real undo runner** — `src/lib/compensation.server.ts` deletes the staged Gmail drafts (new `deleteGmailDraft`), switches the recurring run off, or sends a correction message to the same Telegram chat; expired windows refuse; every undo is written to the task timeline as its own governance event.
  - [x] **Surfaced to the human** — each Decision Card now shows an "If this turns out wrong" line (irreversible ones in red, before you approve), and Governance gained an Undo section listing every recorded reversal with a one-tap Undo. Fully EN/ZH; migration `0008_create_compensations`; 137 library tests passing (8 new), typecheck clean.
- [x] **Phase G.14 — Corrections pass: approval means action, reversibility-based gate, enforced recipients, expiring permissions, revocable proof links (shipped 2026-09-20)** — closes the technical-review work order. Migration `0009_policy_gate_recipients_grants_shares`; 142 library tests passing (5 new gate tests), typecheck clean.
  - [x] **Approval now buys something (Slice A)** — approving an invoice chase *sends* the emails through the owner's live Gmail connection (`runInvoiceChase(..., authorizedSend)`), each send logged to `delivery_logs` with its message id. Without a live connection the chase text is prepared and the outcome says exactly that — no invented send.
  - [x] **The gate judges reversibility, not action names (Slice B)** — `src/lib/policy-gate.ts` replaces the `HIGH_RISK_ACTIONS` / `MEDIUM_RISK_ACTIONS` name lists: every action is classified by reversibility (`not_needed` / `reversible` / `irreversible`) × blast radius (`self` / `customers` / `money`), and unattended running is permitted only for a reversible action confined to the owner. Anything irreversible, anything touching money, and any unknown action pause for a human. `validateIntent` consumes the decision and reports the rule in plain words.
  - [x] **The rule is enforced in the database, not only in code (Slice C)** — new append-only `policy_decisions` table records the decision behind every run (action, reversibility, blast radius, authority, the rule, reasons); trigger `enforce_policy_authority` refuses any row claiming unattended authority for an irreversible or money-reaching action, and `compensations_class_guard` refuses a `reversible` compensation with no reverse action. A future code mistake cannot quietly widen autonomy.
  - [x] **Approved recipients, actually enforced (Slice E)** — `approved_recipients` plus `src/lib/recipients.server.ts`: an authorised chase checks every recipient against the owner's approved list before sending; a non-approved address is `blocked`, logged to `delivery_logs` and reported in the outcome in plain language. New **Approved recipients** page under Trust suggests addresses from existing invoices and leads, and approving is one tap.
  - [x] **Permissions that expire (Slice F)** — `standing_grants` (connection, scope, actions, optional ceiling, 90-day default expiry, revocable) with list/grant/revoke server functions and a one-tap **End now** on the same page.
  - [x] **Proof you can share and switch off (Slice D)** — `receipt_shares` + `GET /api/public/share/:token`: an expiring (14-day default), revocable, view-counted link carrying only the cryptographic evidence — no amounts, names or addresses — alongside the unchanged permanent verify page. Chain anchoring also moved off the approval path: receipts are signed immediately and anchored by the existing 15-minute queue, so approving no longer waits on the chain.
- [x] **Phase G.15 — Approval honesty, provenance labels, refusal log (shipped 2026-09-20)** — PAA priorities 3–5. Migration `0010_create_refusals_log`; 157 library tests passing (15 new), typecheck clean, verified in browser EN + ZH.
  - [x] **Approval honesty (`src/lib/honesty.ts`)** — `checkBatch` refuses any approval that bundles a money-moving action with anything else: one payment per signature, alone. `rubberStampStats` measures the human side — decisions made, refusal rate, how many signed in under five seconds, median time before signing — and returns a verdict (`never_refuses` / `too_fast` / `healthy`). Surfaced on Governance as "Do you ever say no?".
  - [x] **Provenance labels (`src/lib/provenance.ts`)** — every incoming email body is wrapped as UNTRUSTED EXTERNAL TEXT before it reaches the model, with `detectInjection` flagging override attempts, persona swaps, system-prompt probes, urgent-money pressure, credential requests, exfiltration and approval bypass. A flagged message can never produce a reply draft; the run logs a guard event that N messages were treated as data only.
  - [x] **Refusal log (`public.refusals`, RLS own-rows + org)** — one visible record of every refusal: recipient not approved, failed validation, no source to cite, outside text tried to give orders, human rejected, bundled money. Recorded from money chases, triage, Q&A, SIP validation and decision handling; shown on Governance as "Every time it said no".

- [ ] **Phase G.8 — Scoped browser-session adapter (the honest answer to "do Fiverr/Upwork/Taobao for me")** — Tier 3 requests (open-ended work on someone else's platform) are refused today and stay refused in their Instinct-style form: storing the user's real password and driving their account breaches platform terms and is exactly the unbounded-trust model now attracting fines and breach filings. The AiXin version instead: the owner stays signed in themselves in a linked browser session; AiXin acts only inside a named, time-limited scope the owner grants (site, action type, expiry); every step still produces a Decision Card and a signed, anchored receipt; AiXin never holds a password; money moves only with a second confirmation surface. Explicitly out of scope: credential storage, CAPTCHA bypass, and any platform that forbids automation outright. 🔵 Planned — build after G.7, and only with a partner platform that permits it.
